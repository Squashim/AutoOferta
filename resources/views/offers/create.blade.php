<x-app-layout>
    {{-- Tło: bg-gray-100 dark:bg-zinc-800 --}}
    {{-- Czcionka: text-gray-800 dark:text-white --}}
    <section
        class="mx-auto mb-16 mt-4 max-w-4xl rounded-lg bg-gray-100 text-gray-800 shadow-md dark:bg-slate-700 dark:text-white"
    >
        <h1 class="block p-8 text-3xl font-semibold">Kreator nowej oferty</h1>
            ...
            <fieldset class="grid grid-cols-1 gap-4 bg-slate-800 px-8 pb-8 pt-4 md:grid-cols-4">
                ...
                <div class="md:col-span-2">
                    ...
                    >
                        ...
                        <x-radio :label="'Automatyczna'" :nameGroup="'transmission'" :value="'automatic'" />
                    </div>
                </div>
...
{{ old('description') }}</textarea
            ...
            <fieldset class="w-full space-y-4 bg-slate-800 px-8 pb-8 pt-4">
                ...
                <div class="grid grid-cols-3 items-center gap-4">
                    ...
                    <span id="image-count" class="text-right">Liczba zdjęć: 0</span>
                    <input
                        type="file"
                        name="photos[]"
                        id="photos"
                    ...
                    />
                    <div
                        class="col-span-3 flex flex-col gap-3 rounded-md border border-sky-400 bg-sky-500/20 p-4 text-sm"
                        role="alert"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                        ...
                        <p>
                            ...
                            Wymagane od 2 do 5 zdjęć. W celu zmiany zdjęć trzeba je ponownie przesłać. Poprzednie
                            zostaną usunięte!
                        </p>
                    </div>
                </div>
                <div id="image-preview-container" class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
                    {{-- Tutaj pojawia sie zdjecia --}}
                </div>
                        ...
                        <x-accordion.item :label="$category->category_name">
                            @foreach ($category->features as $feature)
                                <x-checkbox
                                    ...
                                    :value="$feature->slug"
                                    :checked="in_array($feature->slug, old('car_features.' . $category->slug, []))"
                                />
                            @endforeach
                        </x-accordion.item>
                    @endforeach
            ...
            <fieldset class="w-full space-y-4 bg-slate-800 px-8 pb-8 pt-4">
                ...
                <div>
                    {{-- Zgoda --}}
                    <label for="terms" class="inline-flex cursor-pointer items-center">
                        <input
                            id="terms"
                            type="checkbox"
                            required
                            ...
                            name="terms"
                        />
                        <span class="ms-2 select-none text-sm">
            ...
            <div class="fixed bottom-0 left-0 w-full bg-gray-100 py-4 shadow-md dark:bg-zinc-900">
                <div class="mx-auto flex max-w-4xl gap-4 px-8">
                    <x-button
                        type="button"
                        onClick="window.location.href='{{route('offers.create')}}'"
                        style="secondary"
                    >
                        Wyczyść
                    </x-button>
                    <x-button type="submit" style="primary">Dodaj ofertę</x-button>
                </div>
            </div>
        </form>
    </section>
...
<script>
    ...
    function fetchModelsHandler() {
        return {
            ...
            selectedModel: {!! json_encode(old('carModel')) !!},
            selectedBrand: {!! json_encode(old('carBrand')) !!},
    ...
    function updateImages(files) {
        ...
        imagesCountContainer.textContent = `Liczba zdjęć: ${imagesCount}`;
        ...
        displayImagePlaceholders(imagesCount);
    }

    document.addEventListener('DOMContentLoaded', () => {
        displayImagePlaceholders(0);
    });
